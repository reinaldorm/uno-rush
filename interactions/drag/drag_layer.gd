...
func end_drag(draggable: Node2D) -> void:
	...
	if draggable.drag_component.drop_zone:
		print("DragLayer: Drag component captured drop_zone and trying to drop...")

		var response = await draggable.drag_component.drop_zone.request_drop(draggable)
		print("DragLayer: After awaiting, `drag_layer` got response= ", response)

		if not response:
			print("DragLayer: Drag component got denied, restoring to parent...")
			restore_to_original_parent(draggable)
		else:
			print("DragLayer: Drag component got accept, resolve left to `drop_zone` owner.")
	else:
		print("DragLayer: Drag component did not captured drop_zone and was ended.\nRestoring to parent.")
		restore_to_original_parent(draggable)
